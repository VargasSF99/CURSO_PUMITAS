...
//  CURSO PUMITAS
//
//  Created by Fernando Vargas Salas on 12/3/19.
...
class CViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    func getAlumnos(){
        ...
        getRef.collection("Pumitas").whereField("Grupo", isEqualTo: "C").order(by: "Tiempo").addSnapshotListener{ (querySnapshot, error) in
    ...
    func getCount() {
        getRef.collection("Pumitas").whereField("Grupo", isEqualTo: "C").getDocuments() { (querySnapshot, err) in
            var totalCount = 0
            if err != nil {
