...
class BViewController: UIViewController,UITableViewDataSource,UITableViewDelegate{
    ...
    func getAlumnos(){
        ...
        getRef.collection("Pumitas").whereField("Grupo", isEqualTo: "B").order(by: "Tiempo").addSnapshotListener{ (querySnapshot, error) in
    ...
    func getCount() {
        getRef.collection("Pumitas").whereField("Grupo", isEqualTo: "B").getDocuments() { (querySnapshot, err) in
            var totalCount = 0
            if err != nil {
